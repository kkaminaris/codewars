...
function fruit(reels, spins){
    // Code here
    let lookUp = ["Null", "Jack", "Queen", "King", "Bar", "Cherry", "Seven", "Shell", "Bell", "Star", "Wild"];
    let values = [];
    let score = 0;
    for (let i=0; i<3; i++) 
      values[i] = lookUp.indexOf(reels[i][spins[i]]);
    values.forEach(e => {
      let same = values.filter(f => f==e).length;
      if (same == 2) {
        let oddOne = values.filter(f => f!=e);
        score = (oddOne==10) ? 2 * e : e;
      }
      if (same == 3)
        score = 10 * e;
    })
