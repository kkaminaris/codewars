...
GuessWho.prototype.guess = function (guess){
  ...
  if (this.character==guess)
    return [ `Correct! in ${this.guesses} turns` ];
  if (this.characteristic.some(([name, _]) => name==guess))
    this.characteristic = this.characteristic.filter(([name, _]) => name!=guess);
  else if (this.characteristic.some(([name, charArr]) => name==this.character && charArr.includes(guess)))
    this.characteristic = this.characteristic.filter(([_, charArr]) => charArr.includes(guess));
  else 
    this.characteristic = this.characteristic.filter(([_, charArr]) => !charArr.includes(guess));
  result = this.characteristic.map(([name, _]) => name);
  return result;
};