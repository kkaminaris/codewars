...
function drive(drinks, finished, drive_time) {
    // Code here
    console.log(drinks);
    let units = drinks.reduce((total, [strength, volume]) => {return total + strength * volume / 1000}, 0).toFixed(2);
    let drive_date = new Date('1970-01-01T' + drive_time + 'Z');
    let finished_date = new Date('1970-01-01T' + finished + 'Z');
    let timeDiff = (drive_date.getTime() - finished_date.getTime())/1000/60/60;
    if (timeDiff<0)
      timeDiff = 24 + timeDiff;
    return [+units, (units<timeDiff)];
}