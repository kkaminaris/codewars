...
function driver(data) {
    // Code here
    let result = data[2].substring(0, 5).toUpperCase().padEnd(5, 9);
    result += new Date(data[3]).getFullYear().toString()[2];
    let temp = data[4] == "M" ? 1 : 51;
    result += (new Date(data[3]).getMonth()+temp).toString().padStart(2, 0);
    result += new Date(data[3]).getDate().toString().padStart(2, 0);
    result += new Date(data[3]).getFullYear().toString()[3];
    result += data[0][0];
    result += data[1] ? data[1][0] : '9';
    result += '9AA';
    return result;
}